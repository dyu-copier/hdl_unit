// ============================================================================
// axi4_master_checker — formal protocol checker for an AXI4 **master** DUT
//
// Binds to the slave-facing side of an AXI4 master:
//   assert()  — checks the DUT's master-driven outputs (requests, write
//               data, response-ready) are protocol compliant
//               (`AXI4_MASTER_RULE`).
//   assume()  — constrains the free-variable slave responses to obey AXI4
//               (`AXI4_SLAVE_RULE`).
//
// The rule set itself lives in axi4_checker_rules.vh, shared verbatim with
// axi4_slave_checker.v (same rules, assert/assume swapped). Rule IDs and
...
// in the .vh header.
//
// Note on capacity: F_MAX_OUTSTANDING / F_MAX_PER_ID bound the checker's
// tracking state. On this side they become ASSERTS on the DUT — a legal
// master that issues deeper pipelines than the bound will trip them; raise
// the parameters to match your design.
//
...
// DUT is the master: master obligations are checked against the DUT
// (assert), slave obligations constrain the environment (assume).
`define AXI4_MASTER_RULE(x) assert(x)
`define AXI4_SLAVE_RULE(x)  assume(x)
`define AXI4_CHK_DUT_IS_MASTER

`include "axi4_checker_rules.vh"

`undef AXI4_MASTER_RULE
`undef AXI4_SLAVE_RULE
`undef AXI4_CHK_DUT_IS_MASTER
